...
for dataset_index, dataset_path in enumerate(datasets):
    ...
    train_dataset, test_dataset = train_test_split(dataset, test_size=0.8, random_state=42)
    ...
    model.load_state_dict(torch.load("fine_tuned_student_model_64Q_DA1_FL1_40_1_20F_1.pth"))

    # Test phase
    model.eval()
    test_features = []

    with tqdm(train_loader, desc=f'Testing {dataset_path}') as pbar_test:
        with torch.no_grad():
...
np.save('all_features_A_LAX.npy', all_features)

# Load all_features from the local file
all_features = np.load('all_features_A_LAX.npy', allow_pickle=True)
